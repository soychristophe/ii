...
let currentSubEnd = 0;
let currentSubStart = 0; // Guardar inicio del subtítulo
let lastSubText = "";
let lastSubStart = -1;
...
function setupPauseBeforeNextSub() {
  ...
  checkInterval = setInterval(() => {
    ...
    if (!isPaused && subVisibility && sid > 0 && nowTime >= (adjustedEnd - pauseMargin) && nowTime < adjustedEnd + 1) {
      ...
      console.log(`Inicio subtítulo: ${currentSubStart.toFixed(2)}s`);
      ...
      if (autoRepeatEnabled) {
        ...
        if (currentRepeatCount < autoRepeatTimes) {
          // Aún quedan repeticiones - hacer seek al inicio del subtítulo
          core.osd(`🔄 Repitiendo ${currentRepeatCount + 1}/${autoRepeatTimes}`);
          isAutoRepeating = true;
          
          // Usar seek directo al timestamp de inicio
          setTimeout(() => {
            mpv.command("seek", [currentSubStart.toString(), "absolute"]);
            console.log(`Haciendo seek a ${currentSubStart.toFixed(2)}s para repetir`);
          }, 100);
        } else {
  ...
  pollInterval = setInterval(() => {
    setTimeout(() => {
      ...
      if (subText && subText.trim() !== "" && subText !== lastSubText) {
        ...
        // Verificar si es el mismo subtítulo
        ...
        currentSubStart = subStart; // Guardar inicio
        ...
        if (isReallyNewSubtitle) {
          ...
          console.log(`*** NUEVO SUBTÍTULO por POLLING: inicio=${subStart.toFixed(2)}s, fin=${currentSubEnd.toFixed(2)}s (contador RESET a 0) ***`);
          isAutoRepeating = false;
        } else {
          lastProcessedSubtitle = subtitleId;
          lastSubStart = subStart;
          console.log(`*** REPETICIÓN por POLLING: inicio=${subStart.toFixed(2)}s, fin=${currentSubEnd.toFixed(2)}s (contador mantiene en ${currentRepeatCount}) ***`);
          isAutoRepeating = false;
        }
...
function handleSubtitleNavigation(command) {
  const sid = mpv.getNumber("sid");
  if (sid <= 0 && command !== "toggle" && command !== "toggle-autorepeat" && command !== "status") {
  ...
  switch(command) {
    case "next":
      ...
      if (!mpv.getFlag("pause")) {
        // Solo reanudar si ya estaba reproduciendo
        startPolling();
      }
      break;
    case "repeat":
      // Hacer seek al inicio del subtítulo actual
      mpv.command("seek", [currentSubStart.toString(), "absolute"]);
      console.log(`*** Repetir: Seek a ${currentSubStart.toFixed(2)}s ***`);
    ...
    case "status":
      ...
      console.log(`Inicio subtítulo actual: ${currentSubStart.toFixed(2)}s`);
      console.log(`========================`);
...
loadSettings(() => {
  console.log("=================================");
  console.log("Plugin de Subtítulos v2.2 Iniciado");
  console.log("=================================");
